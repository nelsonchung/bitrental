...
Map<String, List<String>> citiesData = {
  ...
  '新北市': [
    '板橋區', '三重區', '中和區', '永和區', '新莊區', '新店區', '樹林區', '鶯歌區', '三峽區', '淡水區', '汐止區', '瑞芳區', '土城區', '蘆洲區', '五股區', '泰山區', '林口區', '深坑區', '石碇區', '坪林區', '三芝區', '石門區', '八里區', '平溪區', '雙溪區', '貢寮區',
  ],
  '桃園市': [
  ...
  '台東縣': [
    '台東市', '成功鎮', '關山鎮', '卑南鄉', '鹿野鄉', '池上鄉', '東河鄉', '長濱鄉', '太麻里鄉', '大武鄉', '綠島鄉', '海端鄉', '延平鄉', '金峰鄉', '達仁鄉', '蘭嶼鄉',
  ],
  '花蓮縣': [
    '花蓮市', '鳳林鎮', '玉里鎮', '新城鄉', '吉安鄉', '壽豐鄉', '光復鄉', '豐濱鄉', '瑞穗鄉', '富里鄉', '秀林鄉', '卓溪鄉',
  ],
  '澎湖縣': [
  ...
  void initState() {
    super.initState();
    selectedCity = citiesData[selectedCounty]![0]; // 預設選擇縣市的第一個市/區
  }

  @override
  Widget build(BuildContext context) {
    List<String> cityOptions = citiesData[selectedCounty] ?? [];
          ...
          child: Column(
            ...
            children: [
                  ...
                  DropdownButton<String>(
                    ...
                    onChanged: (newValue) {
                      setState(() {
                        ...
                        selectedCity = citiesData[selectedCounty]![0];
                      });
                    },
                    items: citiesData.keys.map<DropdownMenuItem<String>>(
                      (String value) {
              ...
              Row(
                children: [
                  ...
                  DropdownButton<String>(
                    ...
                    items: cityOptions.map<DropdownMenuItem<String>>(
                      (String value) {
                        return DropdownMenuItem<String>(
                          value: value,
                          child: Text(value),
              ...
              ElevatedButton(
                onPressed: () {
                  // 點擊按鈕後的處理邏輯
                  // 您可以在這裡處理按鈕的點擊事件，例如將選擇的縣市和市/區提交到後端
                },
                child: Text("確認並送出"),
              ),
            ],
